...
class OrdersPostgresConnection:
    ...
    def connect(self) -> psycopg2.extensions.connection:
        try:
            ...
            logger.info(
                "Conexión a PostgreSQL (ms-orders) establecida exitosamente")
            return self.connection
        except Exception as e:
            logger.error(
                f"Error conectando a PostgreSQL (ms-orders): {str(e)}")
            raise
    ...
    def disconnect(self):
        if self.connection:
            ...
            self.connection = None
            logger.info("Conexión a PostgreSQL (ms-orders) cerrada")
